                ...
                withCredentials([sshUserPrivateKey(credentialsId: 'ec2-ssh-key', keyFileVariable: 'SSH_KEY')]) {
                    sh '''
                        ssh -i $SSH_KEY -o StrictHostKeyChecking=no ec2-user@ec2-44-221-45-198.compute-1.amazonaws.com "
                            sudo docker stop monorepo-app || true &&
                            sudo docker rm monorepo-app || true &&
                            sudo docker pull anikb29/monorepo-app:latest &&
                            sudo docker run -d --name monorepo-app -p 4000:4000 anikb29/monorepo-app:latest
                        "
                    '''
